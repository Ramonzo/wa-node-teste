...
class LabController {
    ...
    async update(req, res) {
        if (Array.isArray(req.body)) {
            ...
            async.eachSeries(req.body, function updateObject(obj, done) {
                ...
                //
                try {
                    Lab.updateOne({ _id: obj.id }, { $set: obj }, done);
                } catch {
            ...
            //
            try {
                await Lab.findOneAndUpdate({ _id, deleted: false }, { $set: req.body }, { upsert: false }, function (err, doc) {
                    if (err)
                        return res.status(500).send({ error: '001', message: 'Algo deu errado.' });
                    //
                    return res.status(200).send({ error: '', message: 'Registro atualizado' });
                });
            } catch {
    ...
    async store(req, res) {
        if (Array.isArray(req.body)) {
            ...
            try {
                await Lab.insertMany(
                    ...
                    function (err) {
                        ...
                        //
                        return res.json({ error: '', message: 'Registrados com sucesso.' });
                    }
                );
            } catch {
            ...
            //
            if (!name || !address) return res.status(400).send({ error: 'request_error', message: 'Algum parametro não foi fornecido.' });
            //
            try {
                await Lab.create({ name, address },
                    function (err) {
                        if (err)
                            return res.status(500).send({ error: '001', message: 'Algo deu errado.' });
                        //
                        return res.json({ error: '', message: 'Registrado com sucesso.' });
                    });
            } catch {
    ...
    async index(req, res) {
        try {
            await Lab.find({ deleted: false }, function (err, doc) {
                if (err)
                    return res.status(500).send({ error: '001', message: 'Algo deu errado.' });
                //
    ...
    async get(req, res) {
        ...
        if (!id && !ObjectId.isValid(id))
            return res.status(400).send({ message: 'Algum parametro não foi fornecido.', error: 'request_error' });
        try {
            await Lab.findOne({
                $and:
                    [
    ...
    async delete(req, res) {
        if (Array.isArray(req.body)) {
            ...
            //
            try {
                await Lab.delete({ _id }, function (error) {
                    if (error)
                        return res.status(500).send({ error: '001', message: 'Algo deu errado.' });
                    //
    ...
    async restore(req, res) {
        ...
        //
        try {
            await Lab.restore({ _id }, function (error) {
